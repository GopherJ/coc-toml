...
const agent = process.env.https_proxy
  ? new HttpsProxyAgent(process.env.https_proxy as string)
  : null;
...
`;
  ...
  await new Promise((resolve, reject) => {
    const handle = exec(
      `nix-build -E - --arg src '${origFile}' -o ${dest}`,
      (err, stdout, stderr) => {
        if (err != null) {
          reject(Error(stderr));
        } else {
...
export async function getLatestRelease(): Promise<ReleaseTag | undefined> {
  ...
  const releaseURL =
    'https://api.github.com/repos/kkiyama117/coc-toml/releases/latest';
  // @ts-ignore
  return fetch(releaseURL, { agent })
    .then((resp) => resp.json())
    .then((resp) => {
      const asset = (resp.assets as any[]).find((val) =>
        val.browser_download_url.includes(fix)
      );
...
export async function downloadServer(context: ExtensionContext): Promise<void> {
  workspace.showMessage(`Start searching latest toml release`);
  ...
  if (!latest) {
    statusItem.hide();
    workspace.showMessage(`Can't get latest toml release`);
    return;
  }

  statusItem.text = `Downloading taplo ${latest.tag}`;
  ...
  resp.body.on('data', (chunk: Buffer) => {
    ...
    statusItem.text = `${p}% Downloading taplo ${latest.tag}`;
  });
  ...
  try {
    if (await fs.stat('/etc/nixos')) {
      statusItem.text = `Patching taplo executable...`;
